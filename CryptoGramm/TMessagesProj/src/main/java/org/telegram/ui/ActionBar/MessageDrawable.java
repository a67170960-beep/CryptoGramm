...
public class MessageDrawable extends Drawable {
    ...
    private void generatePath(Path path, Rect bounds, int padding, int rad, int smallRad, int nearRad, int top, boolean drawFullBottom, boolean drawFullTop, boolean customPaint) {
        ...
        if (isOut) {
            ...
            if (currentType == TYPE_MEDIA) {
                ...
                if (drawFullBubble || currentType == TYPE_PREVIEW || customPaint || drawFullBottom) {
                    if (SharedConfig.bubbleTailsEnabled) {
                        ...
                        path.lineTo(bounds.right - padding, bounds.bottom - padding - rad);
            ...
            if (currentType == TYPE_MEDIA) {
                ...
                if (drawFullBubble || currentType == TYPE_PREVIEW || customPaint || drawFullBottom) {
                    if (SharedConfig.bubbleTailsEnabled) {
                        ...
                        path.lineTo(bounds.left + padding, bounds.bottom - padding - rad);
    ...
    public static class PathDrawParams {
        ...
        boolean lastBubbleTailsEnabled;

        public boolean invalidatePath(Rect bounds, boolean drawFullBottom, boolean drawFullTop) {
            boolean invalidate = lastRect.isEmpty() || lastRect.top != bounds.top || lastRect.bottom != bounds.bottom || lastRect.right != bounds.right || lastRect.left != bounds.left || lastDrawFullTop != drawFullTop || lastDrawFullBottom != drawFullBottom || lastBubbleTailsEnabled != SharedConfig.bubbleTailsEnabled || !drawFullTop || !drawFullBottom;
            lastDrawFullTop = drawFullTop;
            lastDrawFullBottom = drawFullBottom;
            lastBubbleTailsEnabled = SharedConfig.bubbleTailsEnabled;
            lastRect.set(bounds);
            return invalidate;
        }
